  ...
  <div>
    ...
    <v-card v-if="recipe.recipeCategory.length > 0 || isEditForm" :class="{'mt-10': !isEditForm}">
      ...
      <v-card-text>
        ...
        />
        <RecipeChips v-else :items="recipe.recipeCategory" v-on="$attrs" />
      </v-card-text>
    </v-card>
    ...
    <v-card v-if="recipe.tags.length > 0 || isEditForm" class="mt-4">
      ...
      <v-card-text>
        <RecipeOrganizerSelector
          ...
          selector-type="tags"
        />
        <RecipeChips v-else :items="recipe.tags" url-prefix="tags" v-on="$attrs" />
      </v-card-text>
    </v-card>
    ...
    <v-card v-if="isEditForm" class="mt-2">
      ...
      <v-card-text class="pt-0">
        <RecipeOrganizerSelector v-model="recipe.tools" selector-type="tools" v-on="$attrs" />
      </v-card-text>
    </v-card>
